...
// A fixed, Tailwind-JIT-safe color palette — payment channels cycle through
// these by position rather than storing raw Tailwind class strings in the
// database (Tailwind only generates CSS for classes it can see in source
// files at build time, so DB-stored class names would silently not render).
const PALETTE = [
  { color: 'border-red-400 bg-red-500/10',    activeColor: 'border-red-500 bg-red-500/20' },
  { color: 'border-blue-400 bg-blue-500/10',   activeColor: 'border-blue-500 bg-blue-500/20' },
  { color: 'border-orange-400 bg-orange-500/10', activeColor: 'border-orange-500 bg-orange-500/20' },
  { color: 'border-green-400 bg-green-500/10', activeColor: 'border-green-500 bg-green-500/20' },
  { color: 'border-purple-400 bg-purple-500/10', activeColor: 'border-purple-500 bg-purple-500/20' },
  { color: 'border-pink-400 bg-pink-500/10',   activeColor: 'border-pink-500 bg-pink-500/20' },
  { color: 'border-cyan-400 bg-cyan-500/10',   activeColor: 'border-cyan-500 bg-cyan-500/20' },
  { color: 'border-amber-400 bg-amber-500/10', activeColor: 'border-amber-500 bg-amber-500/20' },
];
...
export default function Checkout({ cartItems, onUpdateQuantity, onClearCart, onContinueShopping }) {
  ...
  // ── Payment channels + business info — admin-managed, loaded live ────────
  ...
  useEffect(() => {
    (async () => {
      const [{ data: ch }, { data: settings }] = await Promise.all([
        settingsService.fetchPaymentChannels(true),
        settingsService.fetchAppSettings(),
      ]);
      setChannels(ch || []);
  ...
  const BUSINESS_NAME  = businessSettings.business_name || 'Duka Letu';
  const BUSINESS_PHONE = businessSettings.whatsapp_admin_phone || '';

  const totalPrice = cartItems.reduce((s, i) => s + (Number(i.price) * (i.quantity || 1)), 0);
  const methodIdx  = channels.findIndex(m => m.id === selectedMethod);
  const method     = methodIdx >= 0 ? { ...channels[methodIdx], ...PALETTE[methodIdx % PALETTE.length] } : null;
  const methodSteps = method?.instructions
    ? method.instructions.split('\n').map(s => s.replace('{amount}', fmtTZS(totalPrice)).replace('{merchant}', method.merchant_code))
    : [];
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('orders').insert([{
        ...
        payment_method: method?.provider || selectedMethod,
    ...
    <div className="max-w-sm mx-auto text-center py-12 px-4">
      <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-lg">
        ...
        <div className="bg-slate-50 rounded-2xl p-4 text-left space-y-2 text-xs mb-6">
          ...
          <p className="flex justify-between"><span className="text-gray-500">Malipo:</span><span className="font-bold">{method?.provider}</span></p>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            {loadingChannels ? (
              <p className="text-center text-gray-400 text-xs py-6">Inapakia njia za malipo...</p>
            ) : channels.length === 0 ? (
              <p className="text-center text-gray-400 text-xs py-6">Hakuna njia za malipo zilizowekwa bado. Wasiliana na duka moja kwa moja.</p>
            ) : (
                  ...
                  return (
                    <button key={m.id} onClick={() => setSelectedMethod(m.id)}
                      className={`border-2 rounded-2xl p-4 text-left transition-all active:scale-95 ${
                        selectedMethod === m.id ? pal.activeColor + ' scale-[1.02]' : pal.color + ' hover:scale-[1.01]'
                      }`}>
                      <div className="text-2xl mb-2">{m.emoji}</div>
                      <p className="text-xs font-black text-gray-900">{m.provider}</p>
                      <p className="text-[9px] text-gray-500 font-medium">{m.network_label}</p>
                      {selectedMethod === m.id && (
        ...
        <div className="space-y-4">
          ...
          <div className={`border-2 rounded-2xl p-5 ${method.activeColor}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-black text-gray-900 text-sm">{method.provider}</p>
                <p className="text-[10px] text-gray-500">{method.network_label}</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl p-3 mb-4 text-center border border-gray-100">
              ...
              <p className="text-[10px] text-gray-500 mt-1">Kwa: <span className="font-black text-gray-800">{method.business_name}</span></p>
              <p className="text-[10px] text-gray-500">Namba: <span className="font-black text-gray-800">{method.merchant_code}</span></p>
            </div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            ...
            <p className="text-[9px] text-gray-400 mb-3">
              Baada ya kulipa, utapata SMS kutoka {method.network_label} yenye namba ya uthibitisho. Iweke hapa.
            </p>
            <input
