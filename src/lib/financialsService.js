...
export const fetchSalesInRange = (startISO, endISO, cashierId = null) => {
  let q = supabase.from('pos_sales')
    .select('*, pos_sale_items(product_id, quantity, unit_price, discount_amount, line_total, cost_price_at_sale, stock_after_sale, products(name, category, price, barcode)), pos_payments(method, amount)')
...
// Credit sales created within a date range, with the originating sale (for
// sale_no/cashier) and the customer joined in — powers the detailed report's
// "Mauzo ya Deni" table.
export const fetchCreditSalesInRange = (startISO, endISO) =>
  wrap(
    supabase.from('credit_sales')
      .select('*, customers:crm_customers(name, phone), sale:pos_sales(sale_no, cashier_id, created_at)')
      .gte('created_at', startISO).lt('created_at', endISO)
      .order('created_at', { ascending: false })
  );

// Global (all-products) stock movement ledger — every sale/purchase/adjustment/
// transfer/return/reconciliation event, newest first.
...
// Goods-received-note history — what's actually been received against POs,
// with line-level detail (was previously only visible as a stock-count delta).
export const fetchGRNHistory = (limit = 100) =>
  wrap(
    supabase.from('goods_received_notes')
      .select('*, purchase_orders(po_no, suppliers(name)), grn_items(*, products(name, unit))')
