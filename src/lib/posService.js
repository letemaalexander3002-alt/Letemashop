...
export const fetchAllStockMovements = (limit = 200) =>
  wrap(
...
export const searchCustomers = (term) =>
  wrap(supabase.from('crm_customers').select('*').or(`name.ilike.%${term}%,phone.ilike.%${term}%`).limit(20));

export const createCustomer = (customer) =>
  wrap(supabase.from('crm_customers').insert(customer).select().single());
...
export const fetchRecentSales = (limit = 50) =>
  wrap(
    supabase.from('pos_sales').select('*, customers:crm_customers(name, phone), pos_payments(method, amount)')
...
export const fetchSaleDetail = (saleId) =>
  wrap(
    supabase.from('pos_sales')
      .select('*, customers:crm_customers(name, phone), pos_payments(*), pos_sale_items(*, products(name, unit))')
      .eq('id', saleId).single()
  );

// ── RETURNS / REFUNDS ──────────────────────────────────────────────────────────
// ── RETURNS / REFUNDS ──────────────────────────────────────────────────────────
export const fetchSaleByNumber = (saleNo) =>
  wrap(
    supabase.from('pos_sales')
      .select('*, customers:crm_customers(name, phone), pos_sale_items(*, products(name, unit)), pos_returns(id, pos_return_items(sale_item_id, quantity))')
      .ilike('sale_no', `%${saleNo}%`)
      .order('created_at', { ascending: false })
      .limit(10)
  );

export const fetchReturnHistory = (limit = 50) =>
  wrap(
    supabase.from('pos_returns')
      .select('*, pos_sales(sale_no), pos_return_items(*, pos_sale_items(product_id, products(name)))')
      .order('created_at', { ascending: false })
      .limit(limit)
  );
...
// ── STOCK RECONCILIATION (persisted audit trail, not just live adjustments) ──
export const createReconciliationSession = (branchId, performedBy) =>
  wrap(supabase.from('stock_reconciliations').insert({ branch_id: branchId, performed_by: performedBy, status: 'OPEN' }).select().single());

export const submitReconciliationItems = (reconciliationId, rows) =>
  wrap(supabase.from('stock_reconciliation_items').insert(
    rows.map(r => ({ reconciliation_id: reconciliationId, product_id: r.product_id, system_quantity: r.system_quantity, counted_quantity: r.counted_quantity, note: r.note || null }))
  ));

export const closeReconciliation = (id) =>
  wrap(supabase.from('stock_reconciliations').update({ status: 'CLOSED', closed_at: new Date().toISOString() }).eq('id', id));

export const fetchReconciliationHistory = () =>
  wrap(supabase.from('stock_reconciliations').select('*, stock_reconciliation_items(*, products(name))').order('created_at', { ascending: false }).limit(50));
...
export default {
  ...
  fetchRecentSales, fetchSaleDetail, createReturn, createBranch, fetchSaleByNumber, fetchReturnHistory,
  fetchAllStockMovements,
  createReconciliationSession, submitReconciliationItems, closeReconciliation, fetchReconciliationHistory,
};
