...
export const fetchCustomers = (search = '') => {
  let q = supabase.from('crm_customers').select('*').order('created_at', { ascending: false });
  if (search) q = q.or(`name.ilike.%${search}%,phone.ilike.%${search}%,email.ilike.%${search}%`);
  return wrap(q);
};

export const fetchCustomer = (id) =>
  wrap(supabase.from('crm_customers').select('*').eq('id', id).single());

export const createCustomer = (customer) =>
  wrap(supabase.from('crm_customers').insert(customer).select().single());

export const updateCustomer = (id, patch) =>
  wrap(supabase.from('crm_customers').update(patch).eq('id', id).select().single());

export const deleteCustomer = (id) =>
  wrap(supabase.from('crm_customers').delete().eq('id', id));
...
export const fetchCreditSales = () =>
  wrap(
    supabase.from('credit_sales')
      .select('*, customers:crm_customers(name, phone), credit_payments(*)')
