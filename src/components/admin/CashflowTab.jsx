...
export default function CashflowTab() {
  ...
  const [cashMoveTarget, setCashMoveTarget] = useState(null);
  ...
  return (
    <div className="space-y-4">
            ...
            {openSessions.map(s => (
              <div key={s.id} className="bg-slate-900 border border-emerald-500/30 rounded-2xl p-4 flex justify-between items-center flex-wrap gap-2">
                ...
                <div className="flex gap-1.5">
                  <button onClick={() => setCashMoveTarget(s)} className="text-[9px] font-black text-blue-400 bg-blue-500/10 px-3 py-2 rounded-lg uppercase">💵 Ingiza/Toa Fedha</button>
      ...
      {cashMoveTarget && <CashMoveModal session={cashMoveTarget} onClose={() => setCashMoveTarget(null)} onSaved={() => { setCashMoveTarget(null); load(); }} />}
    </div>
  );
}
...
function CashMoveModal({ session, onClose, onSaved }) {
  ...
  const save = async () => {
    if (!(Number(amount) > 0) || !reason.trim()) { setErr('Weka kiasi na sababu.'); return; }
    setSaving(true);
    const { error } = await fin.recordCashMovement(session.id, type, Number(amount), reason.trim());
    setSaving(false);
    if (error) { setErr(error); return; }
    onSaved();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-sm p-5 space-y-3">
        <h3 className="text-sm font-black text-white uppercase">💵 Ingiza / Toa Fedha Rejistani</h3>
        <div className="flex gap-2">
          <button onClick={() => setType('in')} className={`flex-1 py-2 rounded-xl text-xs font-black uppercase ${type === 'in' ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400'}`}>+ Ingiza</button>
          <button onClick={() => setType('out')} className={`flex-1 py-2 rounded-xl text-xs font-black uppercase ${type === 'out' ? 'bg-rose-600 text-white' : 'bg-slate-800 text-slate-400'}`}>− Toa</button>
        </div>
        ...
        <input value={reason} onChange={e => setReason(e.target.value)} placeholder="Sababu (mfano: kununua chai, mabadiliko ya ziada)"
          className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white" />
        {err && <p className="text-rose-400 text-[10px] font-bold">{err}</p>}
        <div className="flex gap-2">
