...
export default function InventoryTab({ products, onReload, currentUser, branchId }) {
  ...
  const loadMovements = useCallback(async () => {
    ...
    const { data } = await pos.fetchAllStockMovements ? await pos.fetchAllStockMovements(200) : { data: [] };
    setMovements(data || []);
  ...
  const lowStock = products.filter(p => p.is_active && !p.is_service && Number(p.stock_quantity) <= Number(p.reorder_level ?? 5));
  ...
  const finishRecon = () => {
    const rows = products.filter(p => p.is_active).map(p => {
      ...
      return { product: p, counted, discrepancy, system: Number(p.stock_quantity) };
  ...
  const applyReconAdjustments = async () => {
    ...
    setMsg('Inasasisha...');
    // 1) open a reconciliation session and persist every counted line (audit trail)
    const { data: session, error: sessErr } = await pos.createReconciliationSession(branchId, currentUser?.id);
    if (!sessErr && session) {
      await pos.submitReconciliationItems(session.id, reconResult.map(r => ({
        product_id: r.product.id, system_quantity: r.system, counted_quantity: r.counted,
      })));
      await pos.closeReconciliation(session.id);
    }
    // 2) apply the actual stock adjustment for each discrepancy
    for (const r of reconResult) {
      await pos.adjustStock(r.product.id, r.discrepancy, 'Marekebisho ya ukaguzi wa stoo (reconciliation)');
    }
    setMsg(`✅ Marekebisho ${reconResult.length} yamehifadhiwa kwenye stoki, rejesta ya ukaguzi wa stoo, na audit log.`);
    setReconResult(null);
    onReload?.();
  };
  ...
  const subTabs = [
    ...
    { id: 'movements', label: '📜 Historia ya Stock' },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-1 bg-slate-900 border border-slate-800 p-1 rounded-xl w-max min-w-full overflow-x-auto">
        {subTabs.map(t => (
          <button key={t.id} onClick={() => t.id === 'recon' ? startRecon() : t.id === 'movements' ? (setSubTab('movements'), loadMovements()) : setSubTab(t.id)}
      ...
      {subTab === 'movements' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
          {loadingMovements ? <div className="py-12 text-center text-slate-600 text-xs">Inapakia...</div> :
          movements.length === 0 ? (
            <div className="py-12 text-center"><p className="text-2xl">📜</p><p className="text-xs text-slate-600 font-bold mt-2">Hakuna miamala ya stoki bado.</p></div>
          ) : (
            <div className="overflow-x-auto max-h-[65vh] overflow-y-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950 border-b border-slate-800 sticky top-0">
                  <tr>{['Muda', 'Bidhaa', 'Aina', 'Mabadiliko', 'Chanzo'].map(h => (
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {movements.map(m => (
                    <tr key={m.id} className="hover:bg-slate-800/30">
                      <td className="px-3 py-2 text-slate-500 whitespace-nowrap">{new Date(m.created_at).toLocaleString('sw-TZ', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</td>
                      <td className="px-3 py-2 text-white font-bold whitespace-nowrap">{m.products?.name || '—'}</td>
                      <td className="px-3 py-2 text-slate-400 uppercase text-[9px] font-bold">{m.movement_type}</td>
                      <td className={`px-3 py-2 font-black whitespace-nowrap ${m.quantity_delta >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {m.quantity_delta >= 0 ? '+' : ''}{m.quantity_delta}
                      </td>
                      <td className="px-3 py-2 text-slate-500 text-[10px]">{m.note || m.reference_type || '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
