...
export default function PurchaseOrdersTab({ products, currentUser, branchId }) {
  ...
  const openGrnHistory = async () => {
    setShowGrnHistory(true); setLoadingGrn(true);
    const { data } = await proc.fetchGRNHistory ? await proc.fetchGRNHistory() : { data: [] };
    setGrnHistory(data || []);
    setLoadingGrn(false);
  };
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <button onClick={openGrnHistory} className="bg-slate-800 hover:bg-slate-700 text-slate-300 text-[10px] font-black px-4 py-2 rounded-xl uppercase">📥 Historia ya GRN</button>
      ...
      {showGrnHistory && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
            <div className="flex justify-between items-center p-5 border-b border-slate-800">
              <h3 className="text-sm font-black text-white uppercase">📥 Historia ya Bidhaa Zilizopokewa (GRN)</h3>
              <button onClick={() => setShowGrnHistory(false)} className="text-slate-500 hover:text-white text-xl">✕</button>
            </div>
            <div className="p-5 space-y-3">
              {loadingGrn ? <p className="text-center text-slate-600 text-xs py-8">Inapakia...</p> :
              grnHistory.length === 0 ? <p className="text-center text-slate-600 text-xs py-8">Hakuna bidhaa zilizopokewa bado.</p> :
              grnHistory.map(g => (
                <div key={g.id} className="bg-slate-950 border border-slate-800 rounded-xl p-3">
                  <div className="flex justify-between items-center mb-1.5">
                    <p className="text-xs font-bold text-white">{g.grn_no}</p>
                    <span className="text-[9px] text-slate-500">{new Date(g.received_at).toLocaleDateString('sw-TZ')}</span>
                  </div>
                  <p className="text-[10px] text-slate-500 mb-1.5">PO: {g.purchase_orders?.po_no} · {g.purchase_orders?.suppliers?.name}</p>
                  {(g.grn_items || []).map(gi => (
                    <p key={gi.id} className="text-[10px] text-slate-400">• {gi.products?.name} — {gi.quantity_received} {gi.products?.unit || 'pcs'} @ {fmtTZS(gi.unit_cost)}</p>
                  ))}
                </div>
              ))}
            </div>
