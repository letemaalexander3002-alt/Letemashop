...
export default function POSTab({ products, onReload, currentUser, branchId, onOptimisticStock, businessInfo }) {
  ...
  const addToCart = (product) => {
    if (!product.is_service && Number(product.stock_quantity) <= 0) { setError(`${product.name} — hakuna stoki.`); return; }
    setError('');
    setCart(prev => {
      ...
      if (idx >= 0) {
        ...
        if (!product.is_service && nextQty > product.stock_quantity) { setError(`Stoki ya ${product.name} ni ${product.stock_quantity} tu.`); return prev; }
  ...
  const updateQty = (productId, qty) => {
    setCart(prev => prev.map(l => {
      ...
      if (!l.product.is_service && q > l.product.stock_quantity) { setError(`Stoki ya ${l.product.name} ni ${l.product.stock_quantity} tu.`); return l; }
      return { ...l, quantity: q };
    }));
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
      ...
      <div className="lg:col-span-3 space-y-3">
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowCameraScan(true)}
            className="px-3 bg-slate-900 border border-slate-800 hover:border-blue-500 rounded-xl text-lg" title="Scan kwa Kamera">
            📷
          </button>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[60vh] overflow-y-auto pr-1">
          {filteredProducts.map(p => (
            <button key={p.id} onClick={() => addToCart(p)} disabled={!p.is_service && p.stock_quantity <= 0}
              ...
              <p className="text-[11px] font-bold text-white truncate">{p.is_service ? '🛎️ ' : ''}{p.name}</p>
              <p className="text-[10px] text-amber-400 font-black mt-1">{fmtTZS(p.price)}</p>
              {p.is_service ? (
                <p className="text-[9px] font-bold mt-0.5 text-purple-400">∞ Huduma</p>
              ) : (
      ...
      {completedSale && (
        <ReceiptModal sale={completedSale} businessInfo={businessInfo} onClose={() => setCompletedSale(null)} />
      )}
      {showCameraScan && (
        <CameraScanModal
          onDetected={(code) => { setShowCameraScan(false); handleBarcodeScanned(code); }}
