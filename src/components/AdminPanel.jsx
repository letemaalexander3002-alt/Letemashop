...
import BarcodeTab from './admin/BarcodeTab';
import ServicesTab from './admin/ServicesTab';
import VariantsModal from './admin/VariantsModal';
...
const EMPTY_PROD = { name:'', category:'', price:'', cost_price:'', stock_quantity:'', barcode:'', reorder_level:'5', description:'', image_url:'', is_active: true };
const EMPTY_CHANNEL = { provider:'', category:'mobile_money', network_label:'', business_name:'', merchant_code:'', emoji:'💳', instructions:'', is_active:true };
...
function ProductsTab({ products, onReload }) {
  ...
  useEffect(() => {
    settingsService.fetchCategories(true).then(({ data }) => {
      setCategories(data || []);
      setForm(p => p.category ? p : { ...p, category: data?.[0]?.name || '' });
    });
  }, []);

  const catNames = categories.map(c => c.name);

  // Services live in the same table but have their own tab/form — keep them
  // out of the physical-inventory list here.
  const physicalProducts = products.filter(p => !p.is_service);

  const filtered = physicalProducts
    .filter(p => catFilter === 'ALL' || p.category === catFilter)
    .filter(p => !search || p.name?.toLowerCase().includes(search.toLowerCase()));

  const openAdd  = () => { setEditProd(null); setForm({ ...EMPTY_PROD, category: catNames[0] || '' }); setDc(false); setShowModal(true); };
  const openEdit = p => {
    setEditProd(p);
    setForm({
      name:p.name, category:p.category, price:String(p.price), cost_price:String(p.cost_price||0),
      stock_quantity:String(p.stock_quantity||0), barcode:p.barcode||'', reorder_level:String(p.reorder_level??5),
      description:p.description||'', image_url:p.image_url||'', is_active:p.is_active!==false,
    });
  ...
  const save = async e => {
    ...
    const pl = {
      name:form.name.trim(), category:form.category, price:Number(form.price), cost_price:Number(form.cost_price||0),
      stock_quantity:Number(form.stock_quantity||0), barcode:form.barcode.trim()||null, reorder_level:Number(form.reorder_level||5),
    ...
    try {
      if (editProd) {
        ...
        if (Number(editProd.price) !== pl.price || Number(editProd.cost_price||0) !== pl.cost_price) {
          const { data: { user } } = await supabase.auth.getUser();
          await supabase.from('audit_logs').insert({
            actor_id: user?.id, action: 'PRICE_OVERRIDE', entity_type: 'products', entity_id: editProd.id,
            before: { price: editProd.price, cost_price: editProd.cost_price }, after: { price: pl.price, cost_price: pl.cost_price },
            note: `${editProd.name}: bei ${editProd.price}→${pl.price}, gharama ${editProd.cost_price||0}→${pl.cost_price}`,
          });
        }
      }
  ...
  const margin = (p) => {
    const price = Number(form.price || (p && p.price) || 0);
    const cost = Number(form.cost_price || (p && p.cost_price) || 0);
    if (price <= 0) return 0;
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-2">
        <h2 className="text-sm font-black text-white uppercase tracking-wide">📦 Bidhaa ({physicalProducts.length})</h2>
        ...
        {filtered.length === 0
          ...
          : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950 border-b border-slate-800">
                  <tr>{['Jina','Kundi','Bei ya Kununua','Bei ya Kuuza','Faida %','Stoki','Hali','Vitendo'].map(h=>(
                    <th key={h} className="px-3 py-3 text-[9px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">{h}</th>
                  ))}</tr>
                </thead>
                <tbody className="divide-y divide-slate-800/60">
                  {filtered.map(p=>{
                    const m = Number(p.price) > 0 ? ((Number(p.price) - Number(p.cost_price||0)) / Number(p.price)) * 100 : 0;
                    return (
                    <tr key={p.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-3 py-3 text-slate-400 font-bold text-[11px] whitespace-nowrap">{fmtTZS(p.cost_price||0)}</td>
                      <td className="px-3 py-3 text-amber-400 font-black text-[11px] whitespace-nowrap">{fmtTZS(p.price)}</td>
                      <td className="px-3 py-3">
                        <span className={`text-[9px] font-black px-2 py-0.5 rounded-md ${m>=30?'bg-emerald-500/10 text-emerald-400':m>=10?'bg-amber-500/10 text-amber-400':'bg-rose-500/10 text-rose-400'}`}>
                          {m.toFixed(0)}%
                        </span>
                      </td>
                      <td className="px-3 py-3">
                      ...
                      <td className="px-3 py-3">
                        <div className="flex gap-1.5">
                          ...
                          <button onClick={()=>setVariantTarget(p)} className="text-[9px] font-black text-purple-400 bg-purple-500/10 hover:bg-purple-500/20 px-2 py-1.5 rounded-lg transition-all">🎨 Aina</button>
                        </div>
                      </td>
                    </tr>
                  );})}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
            ...
            <form onSubmit={save} className="p-5 space-y-3">
              <div>
                <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Jina *</label>
                <input type="text" placeholder="Mfano: Mtandao Saa 1" required value={form.name}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Bei ya Kununua (TZS)</label>
                  <input type="number" placeholder="0" value={form.cost_price}
                    onChange={e=>setForm(p=>({...p,cost_price:e.target.value}))}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-700 focus:outline-none focus:border-blue-500"/>
                </div>
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Bei ya Kuuza (TZS) *</label>
                  <input type="number" placeholder="1000" required value={form.price}
                    onChange={e=>setForm(p=>({...p,price:e.target.value}))}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-700 focus:outline-none focus:border-blue-500"/>
                </div>
              </div>
              {Number(form.price) > 0 && (
                <p className={`text-[10px] font-bold -mt-1 ${margin()>=30?'text-emerald-400':margin()>=10?'text-amber-400':'text-rose-400'}`}>
                  Faida kwa kila mauzo: {fmtTZS(Number(form.price||0)-Number(form.cost_price||0))} ({margin().toFixed(1)}%)
                </p>
              )}

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Stoki</label>
                  <input type="number" placeholder="100" value={form.stock_quantity}
                    onChange={e=>setForm(p=>({...p,stock_quantity:e.target.value}))}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-700 focus:outline-none focus:border-blue-500"/>
                </div>
                <div>
                  <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Kiwango cha Chini</label>
                  <input type="number" placeholder="5" value={form.reorder_level}
              ...
              <div>
                <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">Barcode / SKU</label>
                <input type="text" placeholder="Piga scanner hapa au acha wazi" value={form.barcode}
                  onChange={e=>setForm(p=>({...p,barcode:e.target.value}))}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white placeholder-slate-700 focus:outline-none focus:border-blue-500 font-mono"/>
                <p className="text-[9px] text-slate-600 mt-1">Ukiacha wazi, tumia tab ya "Barcode" kutengeneza na kuchapisha moja kiotomatiki.</p>
              </div>

              {[
                {label:'Maelezo',     key:'description',   type:'text',   ph:'Maelezo mafupi...'},
                {label:'URL ya Picha',key:'image_url',     type:'url',    ph:'https://...'},
              ].map(f=>(
                <div key={f.key}>
      ...
      {variantTarget && <VariantsModal product={variantTarget} onClose={() => setVariantTarget(null)} />}
    </div>
  );
}
...
function CategoriesTab({ products }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, error } = await settingsService.fetchCategories(false);
    if (error) setErr(error); else setCategories(data || []);
    setLoading(false);
  }, []);
  ...
  const addCategory = async () => {
    if (!name.trim()) { setErr('Weka jina la kategoria.'); return; }
    setSaving(true); setErr('');
    const { error } = await settingsService.createCategory({ name: name.trim(), emoji, sort_order: categories.length + 1 });
    setSaving(false);
    if (error) { setErr(error); return; }
    setName(''); setEmoji('📦'); setShowAdd(false); load();
  };

  const toggleActive = async (cat) => {
    await settingsService.updateCategory(cat.id, { is_active: !cat.is_active });
    load();
  };

  const remove = async (cat) => {
    const inUse = products.filter(p => p.category === cat.name).length;
    if (inUse > 0 && !confirm(`Bidhaa ${inUse} zinatumia kategoria hii. Kufuta hakutabadilisha bidhaa hizo, lakini haitaonekana tena kwenye chaguo jipya. Endelea?`)) return;
    const { error } = await settingsService.deleteCategory(cat.id);
    if (error) { setErr(error); return; }
    setDcId(null); load();
  };
  ...
  const stats = categories.map(cat => ({
    ...cat,
    total: products.filter(p => p.category === cat.name).length,
    active: products.filter(p => p.category === cat.name && p.is_active).length,
  }));
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-2">
        ...
        <button onClick={() => setShowAdd(true)} className="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black px-4 py-2 rounded-xl uppercase">+ Ongeza Kategoria</button>
      </div>
      ...
      {loading ? <p className="text-center text-slate-600 text-xs py-8">Inapakia...</p> : (
        <div className="grid grid-cols-1 gap-4">
          {stats.map(cat => (
            <div key={cat.id} className={`bg-slate-900 border rounded-2xl p-5 ${cat.is_active ? 'border-slate-800' : 'border-slate-800 opacity-50'}`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl bg-slate-800">{cat.emoji}</div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xl font-black text-blue-400">{cat.active}</span>
                  <button onClick={() => toggleActive(cat)} className="text-[9px] font-black text-slate-400 bg-slate-800 px-2 py-1 rounded-lg">
                    {cat.is_active ? '✅' : '❌'}
                  </button>
                  {dcId === cat.id ? (
                    <div className="flex gap-1 items-center">
                      <button onClick={() => remove(cat)} className="text-[9px] font-black bg-rose-600 text-white px-2 py-1 rounded-lg">Ndio</button>
                      <button onClick={() => setDcId(null)} className="text-[9px] font-black bg-slate-700 text-slate-300 px-2 py-1 rounded-lg">Hapana</button>
                    </div>
                  ) : (
                    <button onClick={() => setDcId(cat.id)} className="text-[9px] font-black text-rose-400 bg-rose-500/10 px-2 py-1 rounded-lg">🗑</button>
                  )}
                </div>
              </div>
              <div className="w-full bg-slate-800 rounded-full h-1.5">
                <div className="h-1.5 rounded-full bg-blue-500 transition-all"
                  style={{ width: `${cat.total > 0 ? (cat.active/cat.total)*100 : 0}%` }} />
              </div>
          ...
          {categories.length === 0 && <p className="text-center text-slate-600 text-xs py-8">Hakuna kategoria — ongeza ya kwanza hapo juu.</p>}
        </div>
      )}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-sm p-5 space-y-3">
            <h3 className="text-sm font-black text-white uppercase">+ Kategoria Mpya</h3>
            <div className="flex gap-2">
              <input value={emoji} onChange={e => setEmoji(e.target.value)} placeholder="📦"
                className="w-16 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-center text-lg" maxLength={2} />
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Jina la Kategoria"
            ...
            <div className="flex gap-2">
              <button onClick={addCategory} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-black uppercase">{saving ? '...' : 'Hifadhi'}</button>
              <button onClick={() => setShowAdd(false)} className="py-2.5 px-4 rounded-xl bg-slate-800 text-slate-400 text-xs font-black uppercase">Ghairi</button>
            </div>
          </div>
        </div>
      )}
...
function SettingsTab({ orders, products }) {
  ...
  const [dcChannelId, setDcChannelId] = useState(null);
  const [channelErr, setChannelErr] = useState('');

  const loadSettings = useCallback(async () => {
    const { data } = await settingsService.fetchAppSettings();
    setSettings(data || {});
    setForm(data || {});
  }, []);
  ...
  const loadChannels = useCallback(async () => {
    ...
    const { data, error } = await settingsService.fetchPaymentChannels(false);
    if (!error) setChannels(data || []);
    setLoadingChannels(false);
  }, []);

  useEffect(() => { loadSettings(); loadChannels(); }, [loadSettings, loadChannels]);

  const saveInfo = async () => {
    setSavingInfo(true); setInfoMsg('');
    const { error } = await settingsService.upsertManySettings(form);
    setSavingInfo(false);
    if (error) { setInfoMsg('❌ ' + error); return; }
    setInfoMsg('✅ Taarifa za biashara zimehifadhiwa.');
    setSettings(form);
  };

  const openAddChannel = () => { setEditChannel(null); setChForm(EMPTY_CHANNEL); setShowChannelModal(true); setChannelErr(''); };
  const openEditChannel = (c) => {
    setEditChannel(c);
    setChForm({ provider:c.provider, category:c.category, network_label:c.network_label||'', business_name:c.business_name, merchant_code:c.merchant_code, emoji:c.emoji||'💳', instructions:c.instructions||'', is_active:c.is_active });
    setShowChannelModal(true); setChannelErr('');
  };

  const saveChannel = async () => {
    if (!chForm.provider.trim() || !chForm.business_name.trim() || !chForm.merchant_code.trim()) {
      setChannelErr('Jaza jina la mtandao/benki, jina la biashara, na namba ya kulipia.'); return;
    }
    setSavingChannel(true); setChannelErr('');
    const payload = { ...chForm, sort_order: editChannel?.sort_order ?? channels.length + 1 };
    const { error } = editChannel
      ? await settingsService.updatePaymentChannel(editChannel.id, payload)
      : await settingsService.createPaymentChannel(payload);
    setSavingChannel(false);
    if (error) { setChannelErr(error); return; }
    setShowChannelModal(false); loadChannels();
  };

  const removeChannel = async (id) => {
    const { error } = await settingsService.deletePaymentChannel(id);
    if (error) { setChannelErr(error); return; }
    setDcChannelId(null); loadChannels();
  };

  const CATEGORY_LABEL = { mobile_money: 'Pesa za Simu', bank: 'Benki', card: 'Kadi', other: 'Nyingine' };
    ...
    <div className="space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-2.5">
            {[
              ...
              { key: 'business_location', label: 'Eneo' },
              { key: 'business_phone', label: 'Namba ya Simu (Onyesho)' },
              { key: 'whatsapp_admin_phone', label: 'WhatsApp ya Admin (bila +)' },
              { key: 'business_email', label: 'Barua Pepe' },
            ].map(f => (
              <div key={f.key}>
                <label className="block text-[9px] font-black uppercase tracking-wider text-slate-500 mb-1">{f.label}</label>
            ...
            {infoMsg && <p className="text-[10px] font-bold text-center py-1">{infoMsg}</p>}
            <button onClick={saveInfo} disabled={savingInfo} className="w-full py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-black uppercase mt-1">
              {savingInfo ? 'Inahifadhi...' : 'Hifadhi Taarifa za Biashara'}
            </button>
            <p className="text-[9px] text-slate-600">Haya yanaonekana kwenye risiti za POS na ukurasa wa malipo wa duka la mtandaoni.</p>
          </div>
        </div>
      </div>

      {/* PAYMENT CHANNELS — online storefront checkout */}
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
        <div className="flex justify-between items-center border-b border-slate-800 pb-2 mb-4 flex-wrap gap-2">
          <h3 className="text-sm font-black text-white uppercase tracking-wide">💳 Njia za Malipo — Duka la Mtandaoni</h3>
          <button onClick={openAddChannel} className="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-black px-4 py-2 rounded-xl uppercase">+ Ongeza Njia</button>
        </div>
        <p className="text-[10px] text-slate-500 mb-3">Hizi ndizo njia za malipo mteja anachagua kwenye checkout ya duka lako la mtandaoni — kila mtandao/benki ina namba yake ya kulipia (merchant code) na jina la biashara linaloonekana kwa mteja.</p>

        {channelErr && <p className="text-rose-400 text-[10px] font-bold text-center bg-rose-500/10 rounded-lg py-1.5 mb-2">{channelErr}</p>}

        {loadingChannels ? <p className="text-center text-slate-600 text-xs py-6">Inapakia...</p> : (
          <div className="space-y-2">
            {channels.map(c => (
              <div key={c.id} className={`bg-slate-950 border rounded-xl p-3 flex items-center justify-between gap-2 flex-wrap ${c.is_active ? 'border-slate-800' : 'border-slate-800 opacity-50'}`}>
                <div className="flex items-center gap-3">
                  <span className="text-xl">{c.emoji}</span>
                  <div>
                    <p className="text-xs font-bold text-white">{c.provider} <span className="text-slate-600 font-normal">· {CATEGORY_LABEL[c.category] || c.category}</span></p>
                    <p className="text-[10px] text-slate-500">{c.business_name} — {c.merchant_code}</p>
                  </div>
                </div>
                <div className="flex gap-1.5">
                  <button onClick={() => openEditChannel(c)} className="text-[9px] font-black text-blue-400 bg-blue-500/10 px-2 py-1.5 rounded-lg">✏ Hariri</button>
                  {dcChannelId === c.id ? (
                    <>
                      <button onClick={() => removeChannel(c.id)} className="text-[9px] font-black bg-rose-600 text-white px-2 py-1.5 rounded-lg">Ndio</button>
                      <button onClick={() => setDcChannelId(null)} className="text-[9px] font-black bg-slate-700 text-slate-300 px-2 py-1.5 rounded-lg">Hapana</button>
                    </>
                  ) : (
                    <button onClick={() => setDcChannelId(c.id)} className="text-[9px] font-black text-rose-400 bg-rose-500/10 px-2 py-1.5 rounded-lg">🗑 Futa</button>
                  )}
                </div>
              </div>
            ))}
            {channels.length === 0 && <p className="text-center text-slate-600 text-xs py-6">Hakuna njia za malipo — ongeza ya kwanza hapo juu.</p>}
          </div>
        )}
      </div>
      ...
      {showChannelModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto">
            <div className="flex justify-between items-center p-5 border-b border-slate-800">
              <h3 className="text-sm font-black text-white uppercase">{editChannel ? '✏️ Hariri Njia' : '+ Njia Mpya ya Malipo'}</h3>
              <button onClick={() => setShowChannelModal(false)} className="text-slate-500 hover:text-white text-xl">✕</button>
            </div>
            <div className="p-5 space-y-3">
              <div className="flex gap-2">
                <input value={chForm.emoji} onChange={e => setChForm(p => ({ ...p, emoji: e.target.value }))} maxLength={2}
                  className="w-14 bg-slate-950 border border-slate-800 rounded-xl px-2 py-2.5 text-center text-lg" />
                <input value={chForm.provider} onChange={e => setChForm(p => ({ ...p, provider: e.target.value }))} placeholder="Mtandao/Benki (mf. M-Pesa, CRDB Bank)"
                  className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white" />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <select value={chForm.category} onChange={e => setChForm(p => ({ ...p, category: e.target.value }))}
                  className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white">
                  <option value="mobile_money">Pesa za Simu</option>
                  <option value="bank">Benki</option>
                  <option value="card">Kadi</option>
                  <option value="other">Nyingine</option>
                </select>
                <input value={chForm.network_label} onChange={e => setChForm(p => ({ ...p, network_label: e.target.value }))} placeholder="Network (mf. Vodacom)"
                  className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white" />
              </div>
              <input value={chForm.business_name} onChange={e => setChForm(p => ({ ...p, business_name: e.target.value }))} placeholder="Jina la Biashara linaloonekana kwa mteja"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white" />
              <input value={chForm.merchant_code} onChange={e => setChForm(p => ({ ...p, merchant_code: e.target.value }))} placeholder="Namba ya Kulipia / Paybill / Account"
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-white font-mono" />
              <div>
                <label className="block text-[9px] font-black uppercase text-slate-500 mb-1">Hatua za Kulipa (mstari kwa mstari)</label>
                <textarea value={chForm.instructions} onChange={e => setChForm(p => ({ ...p, instructions: e.target.value }))} rows={4}
                  placeholder={'Piga *150*00#\nChagua "Lipa Bili"\nWeka namba: {merchant}\nWeka kiasi: {amount} TZS\nWeka PIN yako kuthibitisha'}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-[11px] text-white font-mono" />
                <p className="text-[9px] text-slate-600 mt-1">Tumia <code className="text-blue-400">{'{merchant}'}</code> na <code className="text-blue-400">{'{amount}'}</code> — vitabadilishwa kiotomatiki.</p>
              </div>
              <label className="flex items-center gap-2 text-xs text-slate-400">
                <input type="checkbox" checked={chForm.is_active} onChange={e => setChForm(p => ({ ...p, is_active: e.target.checked }))} />
                Inaonekana kwa mteja
              </label>
              {channelErr && <p className="text-rose-400 text-[10px] font-bold">{channelErr}</p>}
              <div className="flex gap-2 pt-1">
                <button onClick={saveChannel} disabled={savingChannel} className="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-xs font-black uppercase">
                  {savingChannel ? 'Inahifadhi...' : 'Hifadhi'}
                </button>
                <button onClick={() => setShowChannelModal(false)} className="py-2.5 px-4 rounded-xl bg-slate-800 text-slate-400 text-xs font-black uppercase">Ghairi</button>
              </div>
            </div>
          </div>
        </div>
...
export default function AdminPanel({ onExit }) {
  ...
  const businessName = businessSettings.business_name || 'Letema Stationery & Internet Café';
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await loadBranches();
      const { data: settings } = await settingsService.fetchAppSettings();
  ...
  const lowStockCount = products.filter(p => p.is_active && !p.is_service && Number(p.stock_quantity) <= Number(p.reorder_level ?? 5)).length;
  ...
  const allTabs = [
    ...
    { id:'returns',    label:`↩️ Marejesho`,   badge: 0,             roles:['super_admin','branch_manager','cashier'] },
    { id:'products',   label:`🏷️ Bidhaa`,     badge: 0,             roles:['super_admin','branch_manager','inventory_clerk'] },
    { id:'services',   label:`🛎️ Huduma`,     badge: 0,             roles:['super_admin','branch_manager','inventory_clerk'] },
    { id:'barcode',    label:`🏷️ Barcode`,    badge: 0,             roles:['super_admin','branch_manager','inventory_clerk'] },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        {!loading && (
          <>
            {activeTab === 'pos'        && <POSTab        products={products} onReload={fetchData} currentUser={currentUser} branchId={branchId} onOptimisticStock={applyOptimisticStock} businessInfo={businessSettings} />}
            {activeTab === 'orders'     && <OrdersTab     orders={orders}   onStatusChange={updateOrderStatus} />}
            {activeTab === 'inventory'  && <InventoryTab  products={products} onReload={fetchData} currentUser={currentUser} branchId={branchId} />}
            {activeTab === 'returns'    && <ReturnsTab    currentUser={currentUser} />}
            {activeTab === 'products'   && <ProductsTab   products={products} onReload={fetchData} />}
            {activeTab === 'services'   && <ServicesTab   onReload={fetchData} />}
            {activeTab === 'barcode'    && <BarcodeTab    products={products} onReload={fetchData} businessName={businessName} />}
            ...
            {activeTab === 'financials' && <FinancialsTab products={products} role={role} />}
